...
void print_board(struct Game_T * game, enum Player_T next_player) {
  ...
  ++) {
    ...
    for (int x = 0; x < BOARD_WIDTH; x++) {
      ...
      switch (game->board[x][y]) {
        case PLAYER_NONE:
          r = bruteforce_move_value(game, next_player, x, y, 5);
          if (r == 1)
            putchar('+');
          else if (r == -1)
            putchar('-');
...
int main() {
  struct Game_T * game = new_game();
  //make_move(game, PLAYER_1, 2, 2);
  make_move(game, PLAYER_2, 1, 1);
  make_move(game, PLAYER_2, 1, 3);
  //make_move(game, PLAYER_2, 3, 1);
  make_move(game, PLAYER_2, 3, 3);
  make_move(game, PLAYER_2, 3, 2);
  /*
  make_move(game, PLAYER_2, 3, 2);
  make_move(game, PLAYER_2, 3, 3);
  make_move(game, PLAYER_2, 3, 4);
*/
  print_board(game, PLAYER_1);
/* for (int i = 0; i < 10; i+=2) {
    make_move(game, PLAYER_1, i, 0);
    make_move(game, PLAYER_2, i, 1);
    make_move(game, PLAYER_2, i, 2);
    make_move(game, PLAYER_1, i, 3);
    make_move(game, PLAYER_2, i, 4);
  }
  print_board(game);*/
  return 0;
}
